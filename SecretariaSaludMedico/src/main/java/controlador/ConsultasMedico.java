...
//            }
    ...
    public String autenticar(String correo, String pass) {
        PreparedStatement pst = null;
        ResultSet rs = null;
        try {
            String query = "SELECT * FROM pacientes WHERE correo=? AND pass=?";
            ...
            if (rs.next()) {
                ...
                String token = JWT.create()
                        .withSubject(correo) // Puedes usar CURP como sujeto del token
                        .withExpiresAt(new Date(System.currentTimeMillis() + EXPIRATION_TIME)) // Fecha de expiración del token
                        // Puedes agregar roles u otra información relevante al token si lo deseas
        ...
        // Si no se encontraron filas con las credenciales dadas, devolvemos null
        return null;
    }

    public boolean autenticacion(String cedula, String password) {
        PreparedStatement pst = null;
        ResultSet rs = null;
        try {
            String consulta = "SELECT * FROM medicos WHERE cedula_profesional = ? AND password = ?";
            pst = getConexion().prepareStatement(consulta);
